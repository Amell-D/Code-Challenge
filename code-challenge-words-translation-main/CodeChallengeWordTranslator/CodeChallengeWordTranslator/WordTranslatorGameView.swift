...
struct WordTranslatorGameView: View {
    ...
    private var endgameAlert: Alert {
        return Alert(title: Text("Your final score is: \(viewModel.correctAttempts)"),
                     ...
                     primaryButton: .default(Text("Restart the game"), action: {
            viewModel.correctAttempts = 0
            viewModel.wrongAttempts = 0
        }), secondaryButton: .destructive(Text("Quit the game"), action: {
